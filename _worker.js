 ...
 * ===============================
 */
const WORKER_PASSWORD = ''; // Workers 可直接写死；留空表示必须用 Pages Secret
 ...
 * ===============================
 */
function getRealPassword(env) {
    return env?.[PASSWORD_ENV_KEY] || WORKER_PASSWORD;
}

function isPasswordConfigured(env) {
    const pwd = getRealPassword(env);
...
function checkBasicAuth(request, env) {
    ...
    try {
        ...
        return password === getRealPassword(env);
    } catch {
        return false;
    }
}
 ...
 * ===============================
 */
function getPasswordNotSetHtml() {
    return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
<style>
body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto;background:#020617;color:#e5e7eb;display:flex;align-items:center;justify-content:center;height:100vh}
.box{max-width:480px;padding:32px;border:1px solid #334155;border-radius:16px;box-shadow:0 0 40px rgba(0,0,0,.6);text-align:center}
h1{color:#f87171;font-size:20px;margin-bottom:16px}
p{font-size:14px;line-height:1.7;color:#cbd5f5}
code{display:inline-block;background:#020617;padding:6px 10px;border-radius:6px;color:#38bdf8;margin-top:8px}
</style>
</head>
<body>
<div class="box">
<h1>⚠️ 访问被阻止</h1>
<p>当前服务 <b>未配置访问密码</b>，为防止误暴露，主界面已被禁用。</p>
<p>请在 Cloudflare Pages / Workers 中设置：</p>
<code>ACCESS_PASSWORD</code>
<p style="margin-top:16px;opacity:.7">配置完成后刷新页面即可</p>
</div>
</body>
</html>`;
}
...
function toSuperScript(num) {
    ...
    return num.toString().split('').map(c => s[c] || c).join('');
}
...
export default {
    async fetch(request, env) {
        ...
        // 🚫 未配置密码
        if (!isPasswordConfigured(env)) {
            return new Response(getPasswordNotSetHtml(), {
                status: 500,
...
async function handleGetRegions() {
    try {
        ...
        return new Response(JSON.stringify(Object.keys(counts).sort((a,b)=>counts[b]-counts[a])), {
...
async function handleRawRequest(regionStr, format, limit = 0, requestUrl = null) {
    ...
    const targetRegions = decoded.split(/[,-]/).map(r => r.trim().toUpperCase()).filter(Boolean);

    let needBase64 = false;
    if (requestUrl) {
        const u = new URL(requestUrl);
        needBase64 = u.searchParams.has('base64') && u.searchParams.get('base64') !== '0';
    }

    const response = await fetch("https://zip.cm.edu.kg/all.txt");
    let text = (await response.text()).replace(/^\uFEFF/, '');
    ...
    for (const line of lines) {
        ...
        if (!targetRegions.includes(code)) continue;

        if (limit > 0) {
            const c = (regionLimitCounters[code] || 0) + 1;
            if (c > limit) continue;
            regionLimitCounters[code] = c;
        }
        ...
        if (isCFStyle) {
            ...
            const idx = toSuperScript(regionCounters[code]);
            const port = ipPort.split(':')[1] || '';
            let nodeName = `${flag} ${name}${idx}`;
    ...
    let result = processed.join(format.includes('comma') ? ',' : '\n');
    if (needBase64) result = btoa(unescape(encodeURIComponent(result)));
