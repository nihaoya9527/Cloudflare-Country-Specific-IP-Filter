...
/* ================== 1. 登录系统与全局配置 ================== */
const WORKER_PASSWORD = ''; // 你可以直接在这里写密码，或者在 KV/环境变量中设置 ACCESS_PASSWORD
const PASSWORD_ENV_KEY = 'ACCESS_PASSWORD';
const COOKIE_NAME = 'GXNAS_AUTH';
...
async function handleApiRequest(url) {
  const regions = url.searchParams.get('region')?.split(',') || [];
  const format = url.searchParams.get('format') || 'line';
  // 即使 API 请求 format=line，我们也强制转换成带信息的格式，或者根据你需求修改
  const finalFmt = format === 'line' ? 'cf_line_short' : format;
  return handleRawRequest(regions.join(','), finalFmt, 0, url.toString());
}
    ...
    <main class="max-w-5xl mx-auto px-4 md:px-6 flex flex-col gap-6">
        ...
        <div class="flex items-center gap-2 w-full sm:w-auto z-10">
                    ...
                    <div class="relative group z-20 flex-1 sm:flex-none">
                        <button id="linkBtn" onclick="toggleLinkMenu(event)" class="w-full sm:w-auto bg-slate-900 dark:bg-blue-600 text-white px-5 py-2 rounded-lg font-bold text-xs hover:opacity-80 transition flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20"><i data-lucide="link" size="14"></i> 生成订阅地址</button>
