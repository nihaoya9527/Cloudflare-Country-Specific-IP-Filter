...
/* ================== 2. 工具函数 ================== */
function getPassword(env) { return env?.[PASSWORD_ENV_KEY] || WORKER_PASSWORD; }
function passwordConfigured(env) { return typeof getPassword(env) === 'string' && getPassword(env).length > 0; }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/login') {
      if (request.method === 'POST') {
        ...
        if (fd.get('password') === getPassword(env)) {
          return new Response(null, { status: 302, headers: { 'Set-Cookie': `${COOKIE_NAME}=${btoa(getPassword(env))}; Path=/; HttpOnly; SameSite=Strict`, 'Location': '/' } });
        }
    ...
    if (pathMatches) {
      ...
      // 这里的关键：CFnew 和 edgetunnel 都需要备注，区别只在分隔符
      return handleRawRequest(regions, type, limit, request.url);
    }
...
async function handleGetRegions() {
  try {
    ...
    const counts = {};
    matches.forEach(tag => { const region = tag.replace('#', ''); counts[region] = (counts[region] || 0) + 1; });
...
async function handleApiRequest(url) {
  ...
  return handleRawRequest(regions.join(','), 'edgetunnel', 0, url.toString());
}

async function handleRawRequest(regionStr, type, limit = 0, requestUrl = null) {
  ...
  try {
    ...
    // 分隔符逻辑：CFnew 用逗号，edgetunnel 用换行
    const separator = (type === 'CFnew') ? ',' : '\n';
    let resultStr = processed.join(separator);
    if (needBase64) resultStr = btoa(unescape(encodeURIComponent(resultStr)));

    return new Response(resultStr, { headers: { 'content-type': 'text/plain; charset=UTF-8', 'Access-Control-Allow-Origin': '*' } });
